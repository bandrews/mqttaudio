...
- [ ] Cross-browser (Safari/Firefox/Edge) + manual a11y/visual QA pass recorded in `MANUAL-VERIFICATION.md` `[C]` — *mostly done: **cross-engine parity is now automated-green** — `pnpm test:e2e:crossbrowser` runs the fixture-backed specs + axe a11y across **Chromium (Blink/Edge), WebKit (Safari engine), and Firefox (Gecko)**, 12/12 on all three (real engines, not emulation). The **Chrome** live-daemon pass (all 5 tabs, live telemetry meters+playhead, dark/light themes, tablet-width responsive, keyboard focus, no console errors) is recorded under V-1/V-2/V-3. Genuinely human-reserved remainder: a real **screen-reader** (VoiceOver/NVDA) walk-through, an **ears-on "audio unaffected"** listen, and a visual spot-check in the actual Safari/Edge desktop apps + a mobile browser — assistive tech / human senses this environment can't drive.*
...
- [ ] `MANUAL-VERIFICATION.md` has been run (cross-browser + a11y + visual QA) with results recorded. _**Mostly
  run.** Cross-engine parity is **automated-green**: `pnpm test:e2e:crossbrowser` runs the fixture-backed specs
  + axe a11y across **Chromium (Blink/Edge), WebKit (Safari engine), and Firefox (Gecko)** — 12/12 on all three
  (real engines). The **Chrome** live-daemon pass (all five tabs, live telemetry meters + playhead, dark/light
  themes, tablet-width responsive, keyboard focus, no console errors) is recorded under V-1/V-2/V-3. The
  genuinely **human-reserved remainder** — a real **screen-reader (VoiceOver/NVDA)** walk-through, an ears-on
  "audio unaffected" listen, and a visual spot-check in the actual Safari/Edge desktop apps + a mobile browser —
  needs assistive tech / human senses this environment can't drive; run it before a public release._
